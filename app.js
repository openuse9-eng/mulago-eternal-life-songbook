...
function formatLyrics(lang,h){
  let t=String(h.lyrics||'').trim();
  if(lang==='en' && h.number<=479){
    // The original 1–479 book uses blank lines between every lyric line.
    // Rebuild natural 4-line stanzas while keeping Chorus as its own heading.
    const raw=t.split(/\n\s*\n+/).map(x=>x.trim()).filter(Boolean);
    const out=[]; let verse=[];
    for(const part of raw){
      if(/^chorus:?$/i.test(part)){
        if(verse.length) out.push(verse.join('\n')); verse=[];
        out.push('CHORUS');
      }else{
        verse.push(part);
        if(verse.length===4){out.push(verse.join('\n'));verse=[];}
      }
    }
    if(verse.length) out.push(verse.join('\n'));
    t=out.join('\n\n');
  }else if(lang==='en' || lang==='lg'){
    // Keep each source stanza as a paragraph and make Chorus a clear heading.
    const raw=t.replace(/\\r\\n/g,'\\n').split(/\\n\\s*\\n+/).map(x=>x.trim()).filter(Boolean);
    const out=[];
    for(const part of raw){
      if(/^chorus:?$/i.test(part.trim())){
        out.push('CHORUS');
      }else{
        out.push(part);
      }
...
function lyricsHTML(t){
  return t.split(/\n\s*\n+/).filter(Boolean).map(p=>{
    ...
    if(/^CHORUS$/i.test(p.trim())) return `<p style="margin:0 0 1.15em;font-weight:800"><strong>CHORUS</strong></p>`;
    return `<p style="margin:0 0 1.15em">${safe}</p>`;
  }).join('');
}
let deferredInstallPrompt=null;
const $=s=>document.querySelector(s), $$=s=>document.querySelectorAll(s);
async function load(){state.hymns.en=await fetch('hymns-en.json').then(r=>r.json());state.hymns.lg=await fetch('hymns-lg.json').then(r=>r.json());renderList('en','');renderList('lg','');renderFavorites();renderSermons();}
...
$$('[data-screen]').forEach(b=>b.addEventListener('click',()=>showScreen(b.dataset.screen)));$$('.back').forEach(b=>b.addEventListener('click',()=>showScreen('home')));
$('#menuBtn').addEventListener('click',()=>showScreen('about'));$('#themeBtn').addEventListener('click',()=>document.body.classList.toggle('light'));
...
$('#dismissInstall').addEventListener('click',()=>{localStorage.setItem('melgc-install-dismissed','1');$('#installBanner').hidden=true});window.addEventListener('appinstalled',()=>$('#installBanner').hidden=true);
...
function openHymn(lang,num){const arr=state.hymns[lang],idx=arr.findIndex(h=>h.number===num);if(idx<0)return;state.currentLang=lang;state.currentIndex=idx;const h=arr[idx];$('#readerTitle').textContent=`${h.number}. ${h.title}`;$('#readerKey').textContent=h.key?`Key: ${h.key}`:'';$('#readerBody').innerHTML=lyricsHTML(formatLyrics(lang,h))||'<p>Lyrics not available in the source book.</p>';$('#reader').classList.add('open');$('#reader').setAttribute('aria-hidden','false');updateFavButton()}
$('#closeReader').addEventListener('click',()=>{$('#reader').classList.remove('open');$('#reader').setAttribute('aria-hidden','true')});
function favKey(){return `${state.currentLang}-${state.hymns[state.currentLang][state.currentIndex].number}`}function getFavs(){try{return JSON.parse(localStorage.getItem('melgc-favs')||'[]')}catch{return[]}}function updateFavButton(){const on=getFavs().includes(favKey());$('#favReader').textContent=on?'★':'☆'}
...
function move(d){const arr=state.hymns[state.currentLang];state.currentIndex=(state.currentIndex+d+arr.length)%arr.length;const h=arr[state.currentIndex];$('#readerTitle').textContent=`${h.number}. ${h.title}`;$('#readerKey').textContent=h.key?`Key: ${h.key}`:'';$('#readerBody').innerHTML=lyricsHTML(formatLyrics(state.currentLang,h))||'<p>Lyrics not available in the source book.</p>';updateFavButton();$('#reader').scrollTo(0,0)}
...
function applyFont(){$('#readerBody').style.fontSize=state.font+'px';localStorage.setItem('melgc-font',String(state.font))}$('#smaller').addEventListener('click',()=>{state.font=Math.max(14,state.font-2);applyFont()});$('#larger').addEventListener('click',()=>{state.font=Math.min(34,state.font+2);applyFont()});$('#resetFont').addEventListener('click',()=>{state.font=19;applyFont()});
load().then(()=>applyFont());if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js'));
