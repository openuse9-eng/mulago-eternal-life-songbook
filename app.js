...
function formatLyrics(lang,h){
  ...
  /*
    Preserve braces temporarily because some source
    hymns use them around section information.
  */
  raw=raw
    .replace(/\{[\s]*((?:CHORUS|REFRAIN|VERSE|BRIDGE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)[\s]*:?)\s*\}/gi,
      '\n$1\n')
    .replace(/\[[\s]*((?:CHORUS|REFRAIN|VERSE|BRIDGE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)[\s]*:?)\s*\]/gi,
      '\n$1\n')
    .replace(/\([\s]*((?:CHORUS|REFRAIN|VERSE|BRIDGE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)[\s]*)\)/gi,
      '\n$1\n');

  /*
    Remove braces left over from the original source.
  ...
  */
  ...
  /*
    Remove key information wherever it appears.
  ...
  */
  const keyPattern=
    /\b(?:song\s+key|key\s+signature|key)\s*[:=\-]?\s*[A-G](?:#|b)?(?:\s*(?:m|maj|minor|major|min|sus|dim|aug))?\b/gi;

  raw=raw.replace(keyPattern,'');

  /*
    Remove empty brackets left after removing key information.
  */
  raw=raw
    .replace(/\[\s*\]/g,'')
    .replace(/\(\s*\)/g,'')
  ...
  /*
    Normalise spaces WITHOUT removing new lines.
  */
  raw=raw
    .split('\n')
  ...
  /*
    Remove lines which became empty after Key removal.
  */
  ...
  */
  const heading=
    /^(CHORUS|REFRAIN|BRIDGE|VERSE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)\s*:?[.!]*$/i;

  /*
    Some source data may contain:
      CHORUS: text here
    on one line.

    Split the heading from the lyric text.
  */
  const prepared=[];
  ...
  for(const originalLine of lines){
    ...
    /*
      Detect a heading at the beginning of a line.
    */
    const beginning=
      line.match(/^(CHORUS|REFRAIN|BRIDGE|VERSE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)\s*:?\s*(.*)$/i);

    if(beginning){

      let name=beginning[1].toUpperCase();

      if(/^PRE[- ]?CHORUS$/i.test(beginning[1])){
        name='PRE-CHORUS';
      }
      ...
      if(beginning[2] && beginning[2].trim()){
        prepared.push({
          type:'text',
          text:beginning[2].trim()
        });
      }
    ...
    /*
      Detect headings surrounded by common markers.
    */
    const marked=
      line.match(/^(?:\{|\[|\()\s*(CHORUS|REFRAIN|BRIDGE|VERSE|VAMP|TAG|INTRO|CODA|PRE[- ]?CHORUS)\s*(?:\}|\]|\))$/i);

    if(marked){

      let name=marked[1].toUpperCase();

      if(/^PRE[- ]?CHORUS$/i.test(marked[1])){
        name='PRE-CHORUS';
      }
    ...
    /*
      Remove any accidental Key text that may still remain.
    */
    line=line.replace(keyPattern,'').trim();
  ...
  /*
    ...
    Chorus gets its own .chorus box so the existing CSS
    can style it. Inline fallback styling is also included
    so the chorus remains visibly separated even if the
    CSS class is missing.
  */
  const out=[];

  let current=[];
  let currentSection='stanza';
  ...
  function flush(){
    ...
    if(currentSection==='chorus'){
      ...
      out.push(`
        <div class="chorus" style="
          padding:12px 14px;
          margin:12px 0;
          border-radius:10px;
        ">
          <div class="lyric-heading">CHORUS</div>
          <div class="chorus-text">
            ${clean.map(esc).join('<br>')}
          </div>
        </div>
      `);
  ...
  for(const item of prepared){
    ...
    if(item.type==='heading'){
      ...
      if(name==='CHORUS' || name==='REFRAIN'){

        currentSection='chorus';

        /*
          Do not add a separate heading here.
          The chorus box itself contains the heading.
        */

      }else{

        currentSection='stanza';

        /*
          Keep the other section headings visible.
        */
        out.push(
          `<div class="lyric-heading">${esc(name)}</div>`
        );
  ...
  /*
    If no section was detected, return normal stanzas.
  */
  return out.join('');
}
...
async function load(){
  ...
  try{
    ...
    state.hymns.en=
      await fetch('hymns-en.json').then(r=>{
        if(!r.ok){
          throw new Error('Could not load hymns-en.json');
        }
    ...
    state.hymns.lg=
      await fetch('hymns-lg.json').then(r=>{
        if(!r.ok){
          throw new Error('Could not load hymns-lg.json');
        }
...
function move(d){
  ...
  state.currentIndex=
    (
      state.currentIndex+d+arr.length
    )%arr.length;
