...
(() => {
  ...
  // Bible files can contain simple HTML such as <em>...</em>.
  // Remove markup before searching/displaying it.
  ...
  const verseText = v => typeof v === 'string'
    ? v
    : (v?.text ?? v?.verse ?? v?.content ?? v?.value ?? '');

  const verseNumber = (v,i) => typeof v === 'object' && v
    ? (v.verse ?? v.number ?? v.id ?? i+1)
    : i+1;
    ...
    (data?.books||[]).forEach((book,bi)=>
      (book.chapters||[]).forEach(ch=>
        (ch.verses||[]).forEach((v,i)=>{
          const raw=verseText(v);
          const text=cleanText(raw);
          if(text) out.push({
            book: book.name,
            bookIndex: bi,
            chapter: ch.chapter,
            verse: verseNumber(v,i),
            text
          });
        })
      )
  ...
  function score(item,q,lang){
    const text=normal(item.text);
    const ref=normal(`${item.book} ${item.chapter}:${item.verse}`);
    const query=normal(q);
    let s=0;
    if(ref===query) s+=1000;
    else if(ref.includes(query)) s+=100;
    query.split(' ').filter(x=>x.length>2).forEach(t=>{
      if(normal(item.book).includes(t)) s+=30;
      if(text.includes(t)) s+=8;
    });
    expandTerms(q,lang).forEach(t=>{
      if(t.length>2&&text.includes(t)) s+=3;
    });
    return s;
  }

  // If the user enters an exact reference such as "John 3:16",
  // return the matching verse in both translations at the top.
  function exactReferenceResults(query,lang){
    const m=normal(query).match(/^(.+?)\s+(\d+):(\d+)$/);
    if(!m) return null;

    const bookQuery=m[1], chapter=m[2], verse=m[3];
    let enBookIndex=-1;
    if(state.en){
      const found=state.en.find(x=>normal(x.book)===bookQuery);
      if(found) enBookIndex=found.bookIndex;
    }

    // Also allow a Luganda book name directly.
    let lgBookIndex=-1;
    if(state.lg){
      const found=state.lg.find(x=>normal(x.book)===bookQuery);
      if(found) lgBookIndex=found.bookIndex;
    }
    ...
    const add=(l,index)=>{
      const pool=state[l]||[];
      if(index<0) return;
      const found=pool.find(x=>x.bookIndex===index &&
        String(x.chapter)===chapter && String(x.verse)===verse);
      if(found) results.push({...found,lang:l,score:2000});
    };

    if(lang==='en') add('en',enBookIndex>=0?enBookIndex:lgBookIndex);
    else if(lang==='lg') add('lg',lgBookIndex>=0?lgBookIndex:enBookIndex);
    else {
      // English and Luganda Bibles use the same 66-book order.
      add('en',enBookIndex>=0?enBookIndex:lgBookIndex);
      add('lg',enBookIndex>=0?enBookIndex:lgBookIndex);
    }
  ...
  function renderResults(results,lang){
    ...
    if(!results.length){
      box.innerHTML='<div class="bible-assistant-empty"><b>No matching scripture found.</b><br>Try <b>John 3:16</b>, or a topic such as <b>faith</b>, <b>healing</b>, <b>love</b> or <b>salvation</b>.</div>';
      return;
    }
    ...
    box.querySelectorAll('.bible-assistant-result').forEach(btn=>
      btn.addEventListener('click',()=>openResult(btn.dataset.lang,btn.dataset.book,btn.dataset.chapter,btn.dataset.verse))
    );
  }

  function openResult(lang,book,chapter,verse){
    const item=state[lang]?.find(x=>
      x.book===book && String(x.chapter)===String(chapter) && String(x.verse)===String(verse)
    );
    if(!item)return;
    const reader=el('#bibleReader'),title=el('#bibleReaderTitle'),body=el('#bibleReaderBody');
  ...
  async function search(){
    const input=el('#bibleAssistantInput');
    const lang=el('#bibleAssistantLang')?.value||'both';
    const query=input?.value.trim()||'';
    const status=el('#bibleAssistantStatus');
    if(!query){
      if(status)status.textContent='Type a scripture, reference or topic.';
      return;
    }
    ...
    try{
      ...
      const exact=exactReferenceResults(query,lang);
      if(exact){
        renderResults(exact,lang);
        if(status)status.textContent=`Exact scripture found${lang==='both'?' in English and Luganda.':'.'}`;
        return;
      }

      const pools=lang==='both'
        ? [['en',state.en],['lg',state.lg]]
        : [[lang,state[lang]]];

      const all=[];
      pools.forEach(([l,pool])=>
        pool.forEach(item=>{
          const s=score(item,query,lang);
          if(s>0) all.push({...item,lang:l,score:s});
        })
      );

      all.sort((a,b)=>b.score-a.score);
      renderResults(all,lang);
      ...
      if(status)status.textContent='Bible search could not be loaded. Open the Bible once with internet, then try again.';
    }
  }
  ...
  function installUI(){
    injectStyles();
    const host=el('#bibleLanguages');
    if(!host||el('#bibleAssistant'))return;

    const wrap=document.createElement('div');
    wrap.id='bibleAssistant';
    wrap.className='bible-assistant';
  ...
  if(document.readyState!=='loading')installUI();
})();